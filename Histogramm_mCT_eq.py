...
print(f"µCT Spacing: {ct.GetSpacing()}, Size: {ct.GetSize()}")
...
print(f"µCT Spacing nachher: {ct_04.GetSpacing()}, Size: {ct_04.GetSize()}")
...
ax2.set_xlabel("µCT (HU)")
ax2.set_ylabel("Anzahl Voxel")
ax2.set_title("µCT Histogramm (Knochenmaske)")
...
# µCT: Clipping-Wert ausschließen
hist_ct, bins_ct = np.histogram(arr_ct[arr_mask & (arr_ct < 1950)].flatten(), bins=500)
...
print(f"µCT-Knochenpeak: {hu_knochen:.0f} HU")
...
# Pseudo-CT berechnen
pseudo_ct = slope * arr_mrt + intercept

# Alles außerhalb der Knochenmaske auf -1000 HU setzen
pseudo_ct[~arr_mask] = -1000.0

# Als Bild speichern
result = sitk.GetImageFromArray(pseudo_ct)
result.CopyInformation(mrt_04)  # Spacing, Origin, Direction übernehmen
sitk.WriteImage(result, "/data/u_benndorf_thesis/BA/quick_fun_abstract/v5/pseudo_ct.nrrd")


# Nur Knochenvoxel vergleichen
diff = np.abs(pseudo_ct[arr_mask] - arr_ct[arr_mask])
mae = diff.mean()
rmse = np.sqrt((diff**2).mean())

print(f"MAE: {mae:.1f} HU")
print(f"RMSE: {rmse:.1f} HU")