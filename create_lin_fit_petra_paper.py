...
# Einlesen
mrt = sitk.ReadImage("/data/u_benndorf_thesis/BA/pCT_petra_paper/petra_paper_K_ROI.nrrd")
ct = sitk.ReadImage("/data/u_benndorf_thesis/BA/pCT_seq_0.4/402K_ROI.nrrd")
mask = sitk.ReadImage("/data/u_benndorf_thesis/BA/pCT_petra_paper/bone_mask_petra_paper_final.nrrd")
...
# MRT auf 0.75mm resampeln → wird Referenzgrid
new_spacing = [0.75, 0.75, 0.75]
old_spacing = mrt.GetSpacing()
old_size = mrt.GetSize()
new_size = [int(sz * sp / 0.75) for sz, sp in zip(old_size, old_spacing)]
...
ct_075 = resampler.Execute(ct)
sitk.WriteImage(ct_075, "/data/u_benndorf_thesis/BA/pCT_petra_paper/CT_0.75.nrrd")
mrt_075 = resampler.Execute(mrt)
sitk.WriteImage(mrt_075, "/data/u_benndorf_thesis/BA/pCT_petra_paper/MRT_0.75.nrrd")

# Maske: Nächster-Nachbar-Interpolation, damit sie binär bleibt
resampler.SetInterpolator(sitk.sitkNearestNeighbor)
mask_075 = resampler.Execute(mask)

arr_ct = sitk.GetArrayFromImage(ct_075).astype(np.float32)
arr_mrt = sitk.GetArrayFromImage(mrt_075).astype(np.float32)
arr_mask = sitk.GetArrayFromImage(mask_075) > 0

if arr_mask.ndim == 4:
    arr_mask = arr_mask[:, :, :, 0]

# del ct, mrt, mask, ct_075, mrt_075, mask_075
...
plt.ylabel("CT [HU]")
plt.savefig("/data/u_benndorf_thesis/BA/pCT_petra_paper/lin_plot_petra_paper.pdf", dpi=600)
plt.show()
...
plt.ylabel("CT [HU]")
plt.savefig("/data/u_benndorf_thesis/BA/pCT_petra_paper/lin_plot_petra_paper_poster.png", dpi=600)
plt.show()

# Umrechnung auf komplettes MRT-Array anwenden

# Artefakt-Voxel identifizieren (wahrscheinlich Paddingvoxel von der Registrierung)
artifact_voxels = arr_mrt > 1.5

# Für die pCT-Berechnung: Artefakt-Voxel auf 0 setzen
arr_mrt_clean = np.where(artifact_voxels, 0, arr_mrt)
pct_array = slope * arr_mrt_clean + intercept

# Außerhalb der Maske/an Artefakt-Positionen: Luft (-1000 HU)
pct_array_masked = np.where(arr_mask & ~artifact_voxels, pct_array, -1000)
...
# Speichern als .nrrd (mit Metadaten von mrt_075)
pct_image = sitk.GetImageFromArray(pct_array_masked.astype(np.float32))
pct_image.CopyInformation(mrt_075)  # Spacing, Origin, Direction übernehmen
sitk.WriteImage(pct_image, "/data/u_benndorf_thesis/BA/pCT_petra_paper/pCT_petra_paper_lin.nrrd")
...
axes[2].set_title("CT")
plt.savefig("/data/u_benndorf_thesis/BA/pCT_petra_paper/mrt_pct_ct_plot_lin_axial.pdf")
plt.show()
...
axes[2].set_title("CT")
plt.savefig("/data/u_benndorf_thesis/BA/pCT_petra_paper/mrt_pct_ct_plot_lin_sagittal.pdf")
plt.show()
...
axes[2].set_title("CT")
plt.savefig("/data/u_benndorf_thesis/BA/pCT_petra_paper/mrt_pct_ct_plot_lin_coronal.pdf")
plt.show()
...
ax.set_title("MRI (UCL sequence)")
add_scalebar(ax, spacing_mm=0.75, length_cm=1, img_shape=img_shape)
ax.axis("off")
plt.savefig("/data/u_benndorf_thesis/BA/MRI_paper_poster.png", dpi=600, bbox_inches="tight")
plt.show()
